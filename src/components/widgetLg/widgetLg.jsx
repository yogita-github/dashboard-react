import "./widgetLg.css"

export default function widgetLg() {

  
  return (
    <div className="widgetLg">
        <span className="widgetLgTitle">Latest Transactions</span>
        <ul className="widgetLgList">
            <li className="widgetLgListItem">
                <img src="data:image/jpeg;base64," alt="" className="widgetLgImg" />
                <div className="widgetLgUser">
                    <span className="widgetLgUsername">Vijay Singh</span>
                    <span className="widgetLgUserTitle">Software Engineer</span>
                </div>
                <button className="widgetLgButton">
                    
              Approved

                </button>
            </li>
            <li className="widgetLgListItem">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSoKJPxxwPeNvISnBbZsZHe887Ws0FnrL7o0w&usqp=CAU" alt="" className="widgetLgImg" />
                <div className="widgetLgUser">
                    <span className="widgetLgUsername">Harry William </span>
                    <span className="widgetLgUserTitle">Bank Manager</span>
                </div>
                <button className="widgetLgButton">
                    
                    Declined

                </button>
            </li>
            <li className="widgetLgListItem">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRajqD79cWlZzwzdrDHTCc9PfHuCwFZSSQb0Q&usqp=CAU" alt="" className="widgetLgImg" />
                <div className="widgetLgUser">
                    <span className="widgetLgUsername">Ajay Khan</span>
                    <span className="widgetLgUserTitle">Teacher</span>
                </div>
                <button className="widgetLgButton">
                    
                    Pending

                </button>
            </li>
            <li className="widgetLgListItem">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu7fWIFm7j2Oz0gFHTNp1fskSWJDbi_EUD2Q&usqp=CAU" alt="" className="widgetLgImg" />
                <div className="widgetLgUser">
                    <span className="widgetLgUsername">Smit Adams</span>
                    <span className="widgetLgUserTitle">Student</span>
                </div>
                <button className="widgetLgButton">
                    
                    Declined

                </button>
            </li>
            <li className="widgetSmListItem">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTKvbcMlnxSIWIGaols4Mns0s6e4sZmRftSoA&usqp=CAU" alt="" className="widgetLgImg" />
                <div className="widgetLgUser">
                    <span className="widgetLgUsername">Akash Chavan</span>
                    <span className="widgetLgUserTitle">Accountant</span>
                </div>
                <button className="widgetLgButton">
                    
                    Approved

                </button>
            </li>
        </ul>
        </div>
  )
}
